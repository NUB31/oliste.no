...
export class Paddle extends Sprite2D {
	...
	public constructor(board: Board, upKey: string, downKey: string, color: string) {
		super(new Rect(Vector2D.ZERO, 20, 100), new ColorTexture(color, 10));
		this.upKey = upKey;
	...
	public reset(x: number) {
		this.rect.position = new Vector2D(x, this.board.rect.height / 2 - this.rect.height / 2);
	}

	protected override process(delta: number) {
		if (this.engine.eventHandler.isKeyPressed(this.upKey)) {
			if (this.rect.position.y >= 0) {
				this.rect.position.add(new Vector2D(0, -1), delta);
			}
		}
